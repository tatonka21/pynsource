...
class EditStateDialog(Dialog):
    ...
    def body(self, frame):
        ...
        fsCheckbutton = Checkbutton(
            frame, text="Final State", variable=self.fs, command=self._setForFinal
        )
    ...
    def validate(self):
        if self.name.get() == "":
            ...
            return 0
        elif (self.myState.getName() != self.name.get()) and not self.myStatechart.isFreeName(
            self.myState.parent, self.name.get()
        ):
...
class EditTransitionDialog(Dialog):
    ...
    def body(self, frame):
        ...
        Label(srcDestFrame, text=self.transition.getDest().getName()).grid(
        ...
        Radiobutton(
            eventFrame,
            text="fire on event",
            variable=self.timeEventVar,
            value=1,
            command=self.eventChosen,
        ).pack(side=LEFT)
        ...
        historyCheckbutton = Checkbutton(
            frame, text="Go to the history of the destination state", variable=self.historyValue
        )
...
class GUI:
    ...
    def getStatechartFilename(self):
        return tkinter.filedialog.asksaveasfilename(
            defaultextension="stt",
            filetypes=[("Statechart", ".stt"), ("All files", "*")],
            title="Save as",
        )
    ...
    def getSVMFilename(self):
        return tkinter.filedialog.asksaveasfilename(
            ...
            filetypes=[("SVM Description", ".des"), ("All files", "*")],
            title="Export to SVM",
        )
    ...
    def _createMenuBar(self):
        ...
        self.fileMenu.add_command(
            label="Open...", accelerator="Ctrl+O", command=self.handler.onOpen
        )
        ...
        self.editMenu.add_command(
            label="Add Component Separator...", command=self.handler.onAddSeparator
        )
        self.editMenu.add_command(label="Delete", command=self.handler.onDelete, accelerator="Del")
        self.editMenu.add_separator()
        self.editMenu.add_command(
            label="Initialization Code...", command=self.handler.onEditInitCode
        )
        self.editMenu.add_command(
            label="Interaction Code...", command=self.handler.onEditInteractCode
        )
        self.editMenu.add_command(
            label="Finalization Code...", command=self.handler.onEditFinalCode
        )
    ...
    def _createCanvasPopup(self):
        ...
        self.canvasPopup.add_command(
            label="Add State...", command=self.handler.onAddState, state=NORMAL
        )
        self.canvasPopup.add_command(
            label="Add Transition...", command=self.handler.onAddTransition, state=NORMAL
        )
        self.canvasPopup.add_command(
            label="Add Component Separator...", command=self.handler.onAddSeparator, state=NORMAL
        )

    def _createStatePopup(self):
        self.statePopup = Menu(self.root, tearoff=0)
        self.statePopup.add_command(
            label="Add State...", command=self.handler.onAddState, state=NORMAL
        )
        self.statePopup.add_command(
            label="Add Transition to...", command=self.handler.onAddTransitionTo, state=NORMAL
        )
        self.statePopup.add_command(
            label="Add Component Separator...", command=self.handler.onAddSeparator, state=NORMAL
        )
        ...
        self.statePopup.add_command(
            label="Set as Default", command=self.handler.onSetStateAsDefault, state=NORMAL
        )
        self.statePopup.add_separator()
        # delete state
    ...
    def _createSeparatorPopup(self):
        ...
        self.separatorPopup.add_command(
            label="Delete Component Separator", command=self.handler.onDelete, state=NORMAL
        )

    def _createTransitionPopup(self):
        self.transitionPopup = Menu(self.root, tearoff=0)
        self.transitionPopup.add_command(
            label="Edit...", command=self.handler.onEditTransition, state=NORMAL
        )
        self.transitionPopup.add_command(
            label="Delete", command=self.handler.onDelete, state=NORMAL
        )
